...
export const BentoGridItem = ({
  ...
  spareImg?: string;
    ...
    const handleCopy = () => {
      navigator.clipboard.writeText('js777jacob@gmail.com');
      setcopied(true);
    }
      ...
      <div className={`${id===6 && 'flex justify-center'} h-full`}>
          ...
          {id===6 && (
            <BackgroundGradientAnimation>
              <div className=" absolute z-50 flex items-center justify-center text-white w-full h-full"/>
            </BackgroundGradientAnimation>
          )}
          <div  className={cn(
            titleClassName,
          ...
          {id===3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              ...
              <div className="flex flex-col gap-3 lg:gap-4 pr-4">
                <span className=" py-3 px-3 rounded-lg text-center bg-[#10132e]"/>
                {['AWS', 'MongoDB','TypeScript'].map((item) => (
                  <span key={item} className=" py-2 lg:py-4 lg:px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                    {item}
                  </span>
                ))}
